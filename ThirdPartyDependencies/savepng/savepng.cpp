...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    unsigned width, height, nb_channels;         /* size of matrix */
    ...
    nb_channels = (mxGetNumberOfDimensions(prhs[0])<3)?1:dim_array[2];
    height = mxGetM(prhs[0]);  
    width = mxGetN(prhs[0])/nb_channels;     /* this dimension concatenates with the 3rd dimension */
   
    if((mxGetNumberOfDimensions(prhs[0])>3) || (nb_channels!=1 && nb_channels!=3 && nb_channels!=4)) {
        mexErrMsgIdAndTxt("savepng:nrhs","Input must in the image data format of MxN, MxNx3, or MxNx4 matrix.");
    }
    if((!mxIsUint8(prhs[0]))) {
        mexErrMsgIdAndTxt("savepng:nrhs","Input must be uint8.");
    }
    ...
    imgdata = (unsigned char *)mxMalloc(width * height * nb_channels);
    ...
    for(y = 0; y < height; y++)
    {
        for(x = 0; x < width; x++) 
        {
            ...
            {
                imgdata[idx++] = indata[chan*width*height + x*height + y]; // red, green, blue, alpha
            }
        }
    }
    
    /* Encode PNG in memory */
    outdata = (unsigned char * )tdefl_write_image_to_png_file_in_memory_ex((unsigned char *)imgdata, width, height, nb_channels, dpm, &filelen, max_probes, MZ_FALSE);
