classdef SimplePhaseSLM < SLM
    % Phase SLM class
    %
    % constructor SimplePhaseSLM(displayNumber,referenceDeflectionFrequency,regionOfInterest)
    %
    ...
    %
    % Example
    % slm=SimplePhaseSLM(2,[1 1]/20,[128 128 512 512])
    %
    methods
        function slm=SimplePhaseSLM(displayNumber,referenceDeflectionFrequency,regionOfInterest)
            if (nargin<1)
                displayNumber=[];
            end
            if (nargin<2)
