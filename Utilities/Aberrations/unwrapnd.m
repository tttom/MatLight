% unwrappedPhase=unwrapnd(complexField ,referencePos,posToUnwrap,useSparseMatrix)
% 
...
%
function unwrappedPhase = unwrapnd(complexField, referencePos, posToUnwrap, useSparseMatrix)
    ...
    if nargin<2 || isempty(referencePos),
    ...
    if nargin<4 || isempty(useSparseMatrix),
        useSparseMatrixThreshold=13000;
        % For 2D images this seems to be a reasonable guess of what works
        % fastest. In reality it will strongly depend on how often phase
        % changes occur.
        useSparseMatrix=mean(gradVariability(:))*prod(dataSize)^2>useSparseMatrixThreshold^2;
    end
    clear gradVariability;
            ...
            if nextI>0, % Check if not over edge
                if alreadyUnwrapped(nextI),
                    if magnitude(nextI)>bestMagnitude,
                        ...
                        % Faster than Itoh's Method it seems:
                        referencePhase=unwrappedPhase(nextI)-pi; % Obtain the referencePos unwrapped phase
                        bestPhaseUnwrapped=referencePhase+mod(phase(activeI)-referencePhase,2*pi);
                    end
